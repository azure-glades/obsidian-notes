...
- CPU can only access memory directly from *Registers* and *Cache*. Processes are stored in RAM (memory), and these instructions are loaded sequentially in registers for execution
	...
	- When a process terminates or is moved out of memory, its cache is cleared (i.e set to 0) and stored in the Process Control Block (PCB). This process, called _cache invalidation_, makes room for the next process's cache.
...
- In run-time address binding, the program is bound to virtual memory which is then mapped in runtime by the MMU to physical memory. Hence, logical addresses exist as *virtual addresses* and make up the *virtual address space*. 
...
4. The CPU generates a logical address.
5. *Memory Validation:* The memory management unit (MMU) checks if this logical address is less than the value in the limit register. If it exceeds this limit, an illegal memory access interrupt is raised.
6. *Address Mapping:* If the logical address is valid, the MMU adds the value from the relocation register to the logical address to produce the corresponding physical address.
...
Memory protection in page tables is enforced using **Valid Invalid bits** which determines if a page is within a processes' address space which is called a *namespace*.
